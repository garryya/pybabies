...
# http://ivandemarino.me/2010/01/17/money-change-problem-greedy-vs-dyn-pro/
def make_change(amount, dnm):
    rcl = []
    dnm = sorted(dnm, reverse=True)
    change(amount, dnm, rcl)
    return rcl

def change(amount, dnm, rcl, cl=[]):
    if amount <= 0 or not dnm:
        return
    for i in range(len(dnm)):
        ...
        if dc:
            ...
            dr = amount % d
            if dr:
                change(dr, dnm[i+1:], rcl, cl)
            else:
                rcl.append(cl)
    change(amount, dnm[1:], rcl)
...
if __name__ == '__main__':
    ...
    amount = 4
    dnm = [1, 2, 3]
    rcl = make_change(amount, dnm)
    print('%s changed by %s (%s)\t%s' % (amount, dnm, len(rcl), rcl))
    for c in rcl:
        print('\t%s' % c)